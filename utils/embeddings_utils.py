...
"""
使用 process_item_with_guard 同时提取图片与文本的内容，然后将两者的向量融合成新的向量
    |
    |->如果输入的是图片，那就使用 normalize_image 将本地图片转换为 Base64 编码 ，供多模态嵌入模型使用。
    |->使用call_dashscope_once调用多模态大模型一次，将输入的文本或图像统一编码为 1024 维向量。
"""
...
def process_item_with_guard(item: Dict) -> Dict:
    ...
    #如果是图片输入，则提取图片内
    if image_raw:
        img = normalize_image(image_raw)[0] #把图片文件读取后转换为 Base64 编码的 Data URI 格式（多模态嵌入 API 要求的图片输入格式）
        input_data = [{"image": img, "text": raw_content}] # img是图片的 Data URI 格式，raw_content是图片中的文本内容。
        log.info(f'图片：{image_raw}, 所对应的描述为{raw_content}')

    #如果是文本输入，则获取文本内容raw_content
    else:
        input_data = [{"text": raw_content}]

    ok, embedding, status, retry_after = call_dashscope_once(input_data) #先把之前的图片数据与文本数据放在一起合并后打包成input_data，然后将输入的文本或图像统一编码为 1024 维向量。（阿里云 multimodal-embedding-v1 多模态嵌入模型）
    if ok:
        new_item['dense'] = embedding #把embedding作为新的向量
    else:
        new_item['dense'] = []
