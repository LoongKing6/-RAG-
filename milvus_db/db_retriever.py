...
"""
如果 query 是图片路径：构建图像输入数据，调用多模态嵌入模型生成向量，执行 dense_search。（纯图片只能进行密集向量检索）
如果 query 是文本：构建文本输入数据，同样调用多模态嵌入模型生成向量，执行 hybrid_search（稠密向量 + BM25 稀疏向量混合检索）。
...
class MilvusRetriever:
    ...
    def retrieve(self, query: str) -> List[Dict[str, Any]]:
        if os.path.isfile(query):
            # 构建图像输入数据
            input_data = [{'image': image_to_base64(query)[0]}] #将图片转换为base64编码
            # 调用API获取图像嵌入向量
