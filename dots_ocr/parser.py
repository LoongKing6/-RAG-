...
"""
1，多模态向量建模:使用dots.OCR模型来精准解析PDF和PPT中图片、表格、公式等内容，采用私有化部署gme-Qwen2-VL-7B多模态嵌入模型，将文本与图像映射至统一向量空间，实现Any-to-Any的跨模态检索。
①使用dots.OCR模型来精准解析PDF和PPT中图片、表格、公式等内容：
解析：对应dots_ocr/parser.py
通过load_images_from_pdf将PDF的每一页转换成一张图片，再使用_parse_single_image对单页图像进行 OCR 识别，提取布局信息（文本、图片、表格、公式等），并生成 Markdown 内容。
②文本与图像统一向量化：映射到同一向量空间：
解析：对应utils/embeddings_utils.py
通过 process_item_with_guard 同时提取图片与文本的内容，在此过程中如果输入的是图片，那就使用 normalize_image 将本地图片转换为 Base64 编码，然后将两者的向量融合成新的向量，最后使用call_dashscope_once调用多模态大模型一次，将输入的混合向量编码为 1024 维向量。
③实现Any-to-Any的跨模态检索：
解析：对应milvus_db/db_retriever.py
无论是文本还是图像，都通过同一个多模态嵌入模型生成向量，在同一个向量空间中检索，实现 Any-to-Any 检索
2，文档分块策略优化:通过以章节标题为主导进行递归分块，确保结构完整性;对于超长块采用 SemanticChmkar 语义切割潜代固定的 token 切分。将图片、表格等进行独立分块并转为 HTML/JsON 格式，确保结构完整性。
解析：对应splitters/splitter_md.py
分块后的每个 Document 会自动携带 Header 1、Header 2、Header 3 等元数据，保留文档的结构层级。（这里输入的是由 DotsOCR 解析 PDF 后经过parser.py生成的md文件）。对于文本长度 > 1000 时，进行SemanticChunker语义切分。检测到图片时
3，知识图谱构建:对于存在业务关联的数据，结合 Neo4J 构建知识图谱，提升大模型的推理及回答准确性。
"""
